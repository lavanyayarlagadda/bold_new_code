...
export default function Clients() {
  ...
  const getFilteredClients = () => {
    ...
    if (user?.role === 'staff' || user?.role === 'partner') {
      ...
      const assignedClientIds = [...new Set(assignedServices.map(service => service.clientId))];
  ...
  const getClientStats = (clientId: string) => {
    const clientServices = mockServices.filter(service => service.clientId === clientId);
    return {
      totalServices: clientServices.length,
      activeServices: clientServices.filter(service => 
        service.status === 'in_progress' || service.status === 'not_started'
      ).length,
      completedServices: clientServices.filter(service => service.status === 'completed').length,
      overdueServices: clientServices.filter(service => service.status === 'overdue').length
    };
  };
    ...
    return (
      <tr className="border-b border-gray-100 hover:bg-gray-50">
        ...
        <td className="py-4 px-4 text-center">
          <span className="text-lg font-semibold text-gray-900">{stats.totalServices}</span>
        </td>
        <td className="py-4 px-4">
          <span className={`px-2 py-1 text-xs rounded-full font-medium ${getStatusColor(client.status)}`}>
            {client.status}
    ...
    <div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                {filteredClients.reduce((total, client) => total + getClientStats(client.id).totalServices, 0)}
              </p>
            </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="text-left py-3 px-4 font-medium text-gray-700">Onboarded</th>
      ...
      {filteredClients.length === 0 && (
        <div className="text-center py-12">
          ...
          {(user?.role === 'admin' || user?.role === 'staff') && !searchQuery && (
            <Link
              to="/clients/invite"
