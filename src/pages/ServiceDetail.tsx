...
import { mockServices, mockClients, mockComments, mockDocuments } from '../data/mockData';
  ...
  if (!service) {
    return (
      <div className="text-center py-12">
        ...
        <Link to="/services" className="text-blue-600 hover:text-blue-700 mt-2 inline-block">
          ← Back to Services
        </Link>
      </div>
    );
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className={`px-3 py-1 text-sm rounded-full font-medium ${getStatusColor(service.status)}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="font-medium text-gray-900">{format(service.dueDate, 'PPP')}</p>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="font-medium text-gray-900 capitalize">{service.priority}</p>
                </div>
              </div>
            ...
            <div className="mt-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm font-medium text-gray-900">{service.progress}%</span>
              </div>
                ...
                <div key={task.id} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center justify-between">
                        ...
                        {editingTask === task.id ? (
                          <div className="mt-3 space-y-3">
                            ...
                            <div>
                              ...
                              <select
                                value={taskAssignments[task.id] || task.assignedTo || ''}
                            ...
                            <div>
                              ...
                              <input
                                type="date"
                                value={taskDueDates[task.id] || (task.dueDate ? format(task.dueDate, 'yyyy-MM-dd') : '')}
                                onChange={(e) => handleTaskDueDate(task.id, e.target.value)}
                                className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                              />
                            </div>
                          ...
                          <div className="flex items-center space-x-4 mt-2 text-xs text-gray-500">
                            {task.assignedTo && (
                              <span className="flex items-center">
                                ...
                                {mockStaff.find(s => s.id === task.assignedTo)?.name || 'Staff Member'}
                              </span>
                            )}
                            {task.dueDate && (
                              <span className="flex items-center">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      {(user?.role === 'admin' || user?.role === 'staff') && editingTask !== task.id && (
                        <button
                          onClick={() => {
                            ...
                            setTaskDueDates(prev => ({
                              ...prev,
                              [task.id]: task.dueDate ? format(task.dueDate, 'yyyy-MM-dd') : ''
                            }));
                          }}
                  ...
                  {expandedTasks.includes(task.id) && (
                    <div className="mt-4 pt-4 border-t border-gray-100">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        <div>
                          <p className="text-gray-600 mb-1">Task Details</p>
                          <p className="text-gray-900">Order: #{task.order}</p>
                          <p className="text-gray-900">Required: {task.isRequired ? 'Yes' : 'No'}</p>
                          {task.completionDate && (
                            <p className="text-gray-900">Completed: {format(task.completionDate, 'PPP')}</p>
                          )}
                        </div>
                        <div>
                          <p className="text-gray-600 mb-1">Time Tracking</p>
                          <p className="text-gray-900">Estimated: {task.estimatedHours} hours</p>
                          {task.actualHours && (
                            <p className="text-gray-900">Actual: {task.actualHours} hours</p>
                          )}
                        </div>
                      </div>
                    </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              <div className="flex space-x-3">
                ...
                <div className="flex-1">
                  <p className="text-sm text-gray-900">Bank reconciliation completed by Michael Chen</p>
              ...
              <div className="flex space-x-3">
                ...
                <div className="flex-1">
                  <p className="text-sm text-gray-900">Emma Williams started working on expense categorization</p>
              ...
              <div className="flex space-x-3">
                ...
                <div className="flex-1">
                  <p className="text-sm text-gray-900">Client uploaded bank statements for January</p>
              ...
              <div className="flex space-x-3">
                ...
                <div className="flex-1">
                  <p className="text-sm text-gray-900">Service created and assigned to team</p>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-2">
                  ...
                  <div key={doc.id} className="flex items-center justify-between p-2 hover:bg-gray-50 rounded-lg">
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          {(doc.size / 1024 / 1024).toFixed(1)}MB • {formatDistanceToNow(doc.uploadedAt)} ago
                        </p>
                      </div>
                    </div>
              ...
              {mockDocuments.filter(doc => doc.serviceId === service.id).length === 0 && (
                ...
                .map((comment) => (
                  <div key={comment.id} className="space-y-2">
                    <div className="flex items-start space-x-2">
                      <img
                        src={`https://ui-avatars.com/api/?name=${encodeURIComponent(comment.authorName)}&background=0ea5e9&color=fff`}
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-2">
                          ...
                          {comment.isInternal && (
                            <span className="px-2 py-0.5 text-xs bg-orange-100 text-orange-700 rounded">Internal</span>
                          )}
                        </div>
