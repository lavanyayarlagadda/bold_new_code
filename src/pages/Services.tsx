...
import { mockServices, mockClients, mockServiceTemplates } from '../data/mockData';
...
export default function Services() {
  ...
  const getFilteredServices = () => {
    ...
    if (user?.role === 'client') {
      ...
      filtered = filtered.filter(service => service.assignedTo.includes(user.id));
    }
    ...
    if (clientFilter !== 'all' && (user?.role === 'admin' || user?.role === 'staff')) {
      filtered = filtered.filter(service => service.clientId === clientFilter);
    }
      ...
      <tr className="border-b border-gray-100 hover:bg-gray-50">
        ...
        <td className="py-4 px-4">
          <div className="flex -space-x-1">
            {service.assignedTo.slice(0, 3).map((userId: string, index: number) => (
              <img
                key={userId}
            ...
            {service.assignedTo.length > 3 && (
              <div className="h-6 w-6 rounded-full bg-gray-100 border-2 border-white flex items-center justify-center">
                <span className="text-xs text-gray-600">+{service.assignedTo.length - 3}</span>
              </div>
            )}
          </div>
        </td>
        ...
        <td className="py-4 px-4">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-gray-600 min-w-[35px]">{service.progress}%</span>
          </div>
        </td>
      ...
      {filteredServices.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-4">
            {searchQuery || statusFilter !== 'all' || clientFilter !== 'all' || serviceTypeFilter !== 'all'
